...
function Reports() {
  ...
  const getStatusBadge = (report) => {
    if (report.ventaCerrada || report.analizadoIA) {
      return <span className="px-2 py-1 text-xs rounded-full" style={{ background: 'rgba(27, 127, 74, 0.1)', color: '#1B7F4A' }}>Analizado con IA</span>;
    }
    ...
    <div className="p-8 max-w-full overflow-auto" style={{ background: '#FAFBFC', minHeight: '100vh' }}>
      ...
      {analyzing && analyzeProgress.total > 0 && (
        <div className="mb-4 rounded-xl p-4" style={{
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium" style={{ color: '#1B7F4A' }}>
              Analizando conversaciones con IA...
            </span>
            <span className="text-sm font-semibold" style={{ color: '#1B7F4A' }}>
              {Math.round((analyzeProgress.current / analyzeProgress.total) * 100)}%
            </span>
          </div>
          <div className="w-full rounded-full h-2" style={{ background: 'rgba(27, 127, 74, 0.2)' }}>
            <div
      ...
      {loading ? (
        <div className="flex justify-center py-12">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2" style={{ borderColor: '#1B7F4A' }}></div>
        </div>
      ) : (
              ...
              {filteredReports.length === 0 ? (
                ...
                filteredReports.map((report) => (
                  <tr key={report.id} className={`hover:bg-gray-50 transition-all ${report.isAnalyzing ? 'animate-pulse' : ''} ${analyzedIds.has(report.id) ? 'bg-orange-50' : ''}`}
                    style={report.isAnalyzing ? { background: 'rgba(27, 127, 74, 0.05)' } : {}}
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      <div className="flex items-center">
                        {report.isAnalyzing && (
                          <div className="animate-spin rounded-full h-4 w-4 border-b-2 mr-2" style={{ borderColor: '#1B7F4A' }}></div>
                        )}
        ...
        <div className="mt-6 pt-6 border-t" style={{ borderColor: '#E8EBED' }}>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="rounded-xl p-4" style={{
              ...
              <div className="text-sm" style={{ color: '#1B7F4A' }}>Analizados con IA</div>
              <div className="text-2xl font-semibold text-gray-800">
